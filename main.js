...
function createWindow() {
    try {
        ...
        // A window to load WhatsApp Web. It will be shown for QR code login.
        view = new BrowserWindow({
            show: true, // Show this window for QR code login
            width: 800,
            height: 600,
            webPreferences: {
        ...
        view.webContents.setUserAgent('Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/125.0.0.0 Safari/537.36');
        view.loadURL('https://web.whatsapp.com/');

        // Add listeners for debugging page load issues.
        view.webContents.on('did-fail-load', (event, errorCode, errorDescription) => {
            log(`!!! WhatsApp page failed to load: ${errorDescription} (Code: ${errorCode})`);
        });
        view.webContents.on('crashed', (event, killed) => {
            log(`!!! WhatsApp window crashed. Killed: ${killed}`);
        });
        ...
        view.webContents.on('page-title-updated', (event, title) => {
            if (title === 'WhatsApp' && !isLoggedIn) {
                ...
                log('User is logged in. Hiding WhatsApp window.');
                view.hide(); // Hide the window on successful login
            } else if (title !== 'WhatsApp' && isLoggedIn) {
                isLoggedIn = false;
                mainWindow.webContents.send('status-update', 'Logged Out. Please log in again.');
                log('User is logged out. Showing WhatsApp window.');
                view.show(); // Show the window if user gets logged out
            }
        });
